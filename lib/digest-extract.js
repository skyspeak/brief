...
function validateItems(items, sourceText) {
  for (const item of items || []) {
    if (item.source_url && !urlInSource(item.source_url, sourceText)) {
      ...
      if (!item.source_note) {
        item.source_note = item.source_title || item.source || "Newsletter";
      }
...
export function finalizeDigestMarkdown(parts, { emails, windowDays } = {}) {
  const merged = parts.length === 1 ? parts[0] : mergeDigestResults(parts);
  return digestJsonToMarkdown(merged, { emails, windowDays });
}
...
export function buildScanSummary(emails, insightCount, windowDays) {
  const meta = formatDateRange(emails);
  const n = insightCount ?? 0;
  const windowLabel = windowDays ? `${windowDays}-day window` : "digest window";
  if (!meta) {
    return `Distilled ${n} sharp insight${n === 1 ? "" : "s"} from your ${windowLabel}.`;
  }
  return `Scanned ${meta.count} newsletter${meta.count === 1 ? "" : "s"} in the ${windowLabel} (${meta.range}). Distilled ${n} sharp insight${n === 1 ? "" : "s"}:`;
}

/** Format JSON digest for email / UI (markdown). */
export function digestJsonToMarkdown(data, { emails, windowDays } = {}) {
  ...
  const withUrls = insights.filter((i) => i.source_url).length;
  const scanLine = data.scan_summary || buildScanSummary(emails, insights.length, windowDays);
  ...
  insights.forEach((item, i) => {
    ...
    else if (item.lens && LENS_LABELS[item.lens]) lines.push(`   [${LENS_LABELS[item.lens]}]`);
    lines.push("");
  });
  ...
  if (sources.length) {
    lines.push(
      "---",
      "",
      `All insights grounded in named sources (${sources.slice(0, 8).join(", ")}${sources.length > 8 ? ", …" : ""})` +
        (withUrls ? ` with ${withUrls} verified article URL${withUrls === 1 ? "" : "s"}.` : ".")
    );
  }

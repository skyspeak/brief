// lib/prompts.js — prompt library from docs/newsletter-digest-prompts.md (map-reduce).
...
// §0 Map Step — docs/newsletter-digest-prompts.md
...
// §2 Persona variants — docs/newsletter-digest-prompts.md
...
// §1 Base digest SYSTEM — docs/newsletter-digest-prompts.md
const BASE_DIGEST_SYSTEM = `You are the editor of an executive intelligence digest covering business,
...
// §1 Base digest USER body — docs/newsletter-digest-prompts.md
function buildDigestUser({ runDate, startDate, endDate, n, extracts, personaBlock = "" }) {
  return `Below are extracts from ${n} newsletters published from ${startDate} to ${endDate}.
...
/** §0 Map step — per newsletter at ingest / summarize. */
...
/** §1 + §2 Digest — briefing, scheduled email, preview. */
...
Today's date: ${runDate}. Coverage window: ${startDate} to ${endDate}.`;
  ...
  const personaBlock = neutral ? "" : `${persona.filter}\n\n`;

  return {
    system,
    user: buildDigestUser({ runDate, startDate, endDate, n, extracts, personaBlock }),
  };
}
...
your answer using the digest format:
## 🔭 Top 3 Themes This Cycle · ## 📰 3 Notable Stories · ## 📈 3 Emerging Themes to Watch · ## ✅ 3 Follow-Ups

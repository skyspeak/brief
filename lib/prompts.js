...
Read ALL newsletters below as ONE corpus (a {{WINDOW_DAYS}}-day window). Synthesize across them — find the sharpest cross-newsletter insights. Do NOT summarize each newsletter separately.

Filter out junk (retail promos, "security alert" account emails, sponsor fluff, empty hype). From the substantive remainder, extract insights an executive would forward to a colleague.
...
- Grounded in a named newsletter (publication from the header, e.g. "Import AI 464", "TLDR AI").
...
URL RULES (critical — every insight must be link-grounded when possible):
- source_url: the ORIGINAL article/report URL that the newsletter links to for this story — NOT view-in-browser, unsubscribe, sponsor, or newsletter homepage links.
- URLs in newsletters appear as "label (https://...)" — extract the https URL from the text.
- Only use URLs that literally appear in the newsletter input. Never invent or guess URLs.
- If the newsletter discusses a story and includes its article URL, you MUST set source_url to that URL.
- Strip utm_*, ref, fbclid, mc_*, gclid tracking params mentally; return the clean canonical URL.
- If no article URL exists for a story, set source_url to null.
  ...
  "insights": [
    {
      ...
      "source_url": "<article URL from newsletter text, or null>",
      "lens": "engineering"
    }
  ]
}
...
/** Unified cross-newsletter digest for the configured window. */
export function buildDigestExtractPrompt({ content, runDate, windowDays }) {
  const days = windowDays ?? Number(process.env.DIGEST_WINDOW_DAYS || process.env.DIGEST_INTERVAL_DAYS || 3);
  let user = DIGEST_UNIFIED_USER.replace("{{WINDOW_DAYS}}", String(days));
  user = user.replace("{{NEWSLETTER_CONTENT}}", content).replace(
    "<YYYY-MM-DD>",

...
export function buildDigestPrompt({ personaKey, runDate, startDate, endDate, n, extracts }) {
  ...
  const system = `You are the editor of an executive intelligence digest read by ${persona.role}.
...
export const PERSONA_OPTIONS = Object.entries(PERSONA_PROMPTS).map(([id, p]) => ({
  id,
  label: p.role,
}));

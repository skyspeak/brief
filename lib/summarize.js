...
import { prepareNewsletterContentForDigest, cleanNewsletterContent } from "@/lib/newsletter-text";
...
export async function summarizeEmail({ subject, sender, body, body_html, receivedAt } = {}) {
  ...
  const text = prepareNewsletterContentForDigest(cleaned, { subject, sender, receivedAt, body_html });

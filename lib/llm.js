...
async function openRouterChat({ model, system, user, maxTokens, json }) {
  const maxAttempts = 4;
...
export async function callLLM({ system, user, maxTokens = 2400, json = false }) {
  ...
  if (PROVIDER === "openrouter") {
    return openRouterChat({ model, system, user, maxTokens, json });
  }

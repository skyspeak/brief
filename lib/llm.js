...
export function getProviderChain() {
  ...
  let fallback = process.env.LLM_FALLBACK || null;
  if (!fallback && primary !== "openrouter" && process.env.OPENROUTER_API_KEY) {
    fallback = "openrouter";
  }
...
export function resolveModelFor(provider) {
  ...
  if (process.env.LLM_MODEL && (!process.env.LLM_PROVIDER || process.env.LLM_PROVIDER === provider)) {
...
export async function callLLM({ system, user, maxTokens = 2400, json = false, maxAttempts }) {
  ...
  if (chain.length === 0) {
    throw new Error("No LLM configured — set GEMINI_API_KEY, ANTHROPIC_API_KEY, or OPENROUTER_API_KEY");
  }

...
export async function callLLM({ system, user, maxTokens = 2400, json = false, maxAttempts }) {
  ...
  if (primaryErr && chain.length > 1 && lastErr !== primaryErr) {
    ...
    const hint =
      fb === "openrouter"
        ? " Remove LLM_FALLBACK=openrouter from Vercel (Gemini-only is recommended)."
        : "";
    throw new Error(`${primaryErr.message} (${fb} fallback also failed: ${lastErr.message})${hint}`);
  }
  throw lastErr;
}

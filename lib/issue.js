...
/** @returns {Promise<string>} neutral digest markdown (talking points, stats, persona insights) */
export async function buildDigest(emails, _personaKey = "neutral", _windowDays) {
  const { markdown } = await buildDigestFromEmails(emails);
  return markdown;
}

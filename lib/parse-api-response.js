...
export function apiError(parsed, fallback = "request failed") {
  ...
  if (typeof fromData === "string") {
    ...
    if (/OpenRouter.*429|openrouter.*rate.?limit/i.test(fromData)) {
      return (
        "OpenRouter rate limited (Gemini failed first). Remove LLM_FALLBACK=openrouter " +
        "and OPENROUTER_API_KEY from Vercel — use Gemini only."
      );
    }

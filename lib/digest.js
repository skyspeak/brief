...
/** Newsletters with usable body, excluding confirmations. */
export function filterDigestEmails(emails) {
  return emails.filter(
    (e) =>
      !isConfirmationEmail(e) &&
      e.tags !== "confirmation" &&
      (e.body_text || "").trim().length > 120
  );
}

export async function getDigestWindowEmails({ limit = DIGEST_NEWSLETTER_LIMIT } = {}) {
  const { windowDays, now, since } = digestWindowConfig();
  const inWindow = filterDigestEmails(await recentEmails(since));
  const capped = capNewslettersForDigest(inWindow, limit);
  return { ...capped, windowDays, now };
}

/** Briefing uses the same digest window (not the full corpus). */
export async function getBriefingEmails({ limit = DIGEST_NEWSLETTER_LIMIT } = {}) {
  return getDigestWindowEmails({ limit });
}
...
/** Newsletters that already have map-step summaries (legacy fallback). */
export async function getEmailsWithSummaries({ limit = DIGEST_NEWSLETTER_LIMIT } = {}) {
  const { emails: inWindow } = await getDigestWindowEmails({ limit: 500 });
  return capNewslettersForDigest(inWindow.filter(hasUsableSummary), limit);
}
